...
import * as Yup from "yup";
   ...
   const  FormSyafası = ()  => {
    ...
    const [form, setForm] = useState({
      ...
      const formSchema = Yup.object().shape({
        name: Yup.string().required("Lütfen isminizi giriniz..").min(2,"en az iki karakter içermelidir."),
        surname: Yup.string().required("Lütfen soyadınızı giriniz..").min(2,"en az iki karakter içermelidir."),
        password: Yup.string().required("Lütfen şifrenizi giriniz..").min(6,"en az altı karakter içermelidir."),
        email: Yup.string(),
        check : Yup.boolean().oneOf([true],"Kullanıcı bilgilerinin paylaşılmasını onaylayın. "),
      })
  ...
  const handleSubmit = (e) => {
    ...
    axios.get("https://reqres.in/api/users").then(res => console.log(res.data));
    });
  }  
  ...
  return (
    ...
    <div>
      ...
      <Form onSubmit={handleSubmit}>
        ...
        <button type='submit' disabled={!formDolumu}>Gönder!</button>
